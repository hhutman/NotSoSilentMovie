...
//, {
    ...
    socket.on('streamStatus', function (data) {
        ...
        if (data.streamRunningPhilly) {
            playerInstance.load([{file: "rtmp://45.79.70.119/live/philly2"}]);
        } else if (!data.streamRunningPhilly) {
    ...
    socket.on('changeStreamPhilly', function (data) {
        console.log('change stream ' + data);
        if (data == 'startphilly') {
            playerInstance.load([{file: "rtmp://45.79.70.119/live/philly"}]);
